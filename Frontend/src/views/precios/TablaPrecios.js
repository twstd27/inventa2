...
const TablaPrecios = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: 'percent',
        header: 'PORCENTAJE SOBRE PRECIO BASE',
        cell: ({ row }) => (row.original.percent ? `${row.original.percent}%` : '0%'),
      },
      {
    ...
    <>
      ...
      <CPagination aria-label="Page navigation" size="sm" className="cursor-pointer">
        ...
        <div className="w-100 d-flex justify-content-between">
          <span className="pt-1 mx-2">{`Página ${pagination.pageIndex + 1} de ${Math.ceil(listasPrecios.length / pagination.pageSize)}`}</span>
