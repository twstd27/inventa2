...
const TablaSalidas = () => {
  ...
  const { salidas, ultimaPagina, totalSalidas, setEntrada, getSalidas } = useStockStore()
  ...
  const openModal_ = (registro) => {
    setEntrada(registro)
    openModal(
      <span><CIcon icon={cilPencil} /> Editar salida de inventario</span>,
      'Guardar Cambios',
      { action: 'modificar', type: 'salida' },
    )
  }
  ...
  const toggleAlert = (accion, registro) => {
    ...
    openDialog(
      ...
      <span>Está seguro que quiere cambiar la salida de inventario a estado <strong>{accion}</strong>?</span>,
      'Si', 'No', accion,
    )
  }
